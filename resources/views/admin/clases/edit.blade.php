...
@section('title', 'IBBSC - Editar Clase')
@section('page-title', 'Editar Clase de Asistencia')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.clases.update', $clase) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Slug (generado automáticamente)</label>
                    <input type="text" value="{{ $clase->slug }}" readonly
                           class="w-full rounded-md border-gray-300 bg-gray-50 text-gray-500 shadow-sm font-mono">
                    <p class="mt-1 text-sm text-gray-500">Este slug se usa internamente en la base de datos</p>
                </div>

                <div>
                    <label for="color" class="block text-sm font-medium text-gray-700 mb-2">Color *</label>
                    <div class="flex items-center gap-3">
                        <input type="color" name="color" id="color" value="{{ old('color', $clase->color) }}" 
                               class="h-10 w-20 rounded border-gray-300 shadow-sm" required>
                        <span id="colorHex" class="text-sm text-gray-600">{{ old('color', $clase->color) }}</span>
                    </div>
                    @error('color')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="orden" id="orden" min="0" value="{{ old('orden', $clase->orden) }}" 
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="tiene_maestros" id="tiene_maestros" value="1" {{ old('tiene_maestros', $clase->tiene_maestros) ? 'checked' : '' }}
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="activa" id="activa" value="1" {{ old('activa', $clase->activa) ? 'checked' : '' }}
                           class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <label for="activa" class="ml-2 text-sm text-gray-700">
                        Clase activa (aparece en el formulario de asistencia)
                    </label>
                </div>
            </div>
