...
class IngresosAsistenciaController extends Controller
{
    public function index()
    {
        ...
        $categorias = [
            ...
            'campamento' => $cultosSemanales->sum(fn($c) => $c->totales ? $c->totales->total_campa : 0),
            'pro-templo' => $cultosSemanales->sum(fn($c) => $c->totales ? $c->totales->total_construccion : 0),
            'ofrenda_especial' => $cultosSemanales->sum(fn($c) => $c->totales ? $c->totales->total_micro : 0),
        ...
        if ($tipoReporte == 'culto') {
            foreach ($cultos as $culto) {
                if ($culto->totales) {
                    $registros[] = [
                        ...
                        'campamento' => $culto->totales->total_campa,
                        'pro-templo' => $culto->totales->total_construccion,
                        'ofrenda_especial' => $culto->totales->total_micro,
            ...
            foreach ($semanas as $semana => $cultosSeamana) {
                $registros[] = [
                    ...
                    'campamento' => $cultosSeamana->sum('totales.total_campa'),
                    'pro-templo' => $cultosSeamana->sum('totales.total_construccion'),
                    'ofrenda_especial' => $cultosSeamana->sum('totales.total_micro'),
            ...
            foreach ($meses as $mes => $cultosMes) {
                ...
                $registros[] = [
                    ...
                    'campamento' => $cultosMes->sum('totales.total_campa'),
                    'pro-templo' => $cultosMes->sum('totales.total_construccion'),
                    'ofrenda_especial' => $cultosMes->sum('totales.total_micro'),
